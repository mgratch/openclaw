...
- The individual `POST /pulls/{n}/comments` endpoint with `-f`/`-F` flags — it's hunk-restricted by design. Always use the reviews endpoint.
- The `position` field (legacy, diff-relative, constrains you to the hunk)

## Step-by-step

### 1. Verify file paths exist in the PR

```bash
gh pr diff PR_NUMBER --name-only
```

Paths in your review payload must match this list exactly.
...
### 3. Write the review payload to a file
...
- `\n` in JSON string values is correct and expected — it renders as a real newline in the posted comment. This is different from `gh issue comment -b` body flags (see below).
...
|---|---|---|
| `line must be part of the diff` | Using the individual `/pulls/{n}/comments` endpoint, or using the `position` field | Switch to `POST /pulls/{n}/reviews` with `line`/`side` in the `comments` array |
...
| `Validation Failed` without specifics | JSON structure is wrong at a level the API can't name | Validate with `jq . /tmp/review.json` and check against the schema above |
...
# Handle GitHub text safely (for issue/PR body comments, not review comments)

These rules apply to `gh issue comment`, `gh pr comment`, and `gh issue create` / `gh pr create` body flags — **not** to the review-comments JSON payload above, where `\n` in string values is correct.
...
- Before posting a PR review, run `gh pr diff PR_NUMBER --name-only` to verify file paths exist in the PR.
